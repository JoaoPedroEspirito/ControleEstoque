...
-- Tabela de relacionamento Cliente-Produto (Compras)
CREATE TABLE ClienteProduto (
    ClienteId INT NOT NULL,
    ProdutoId INT NOT NULL,
    DataUltimaCompra DATETIME,
    QuantidadeComprada INT DEFAULT 1,
    PRIMARY KEY (ClienteId, ProdutoId),
    FOREIGN KEY (ClienteId) REFERENCES Cliente(Id),
    FOREIGN KEY (ProdutoId) REFERENCES Produto(Id)
);

-- Tabela de relacionamento Fornecedor-Produto (Fornecimento)
CREATE TABLE FornecedorProduto (
    FornecedorId INT NOT NULL,
    ProdutoId INT NOT NULL,
    PrecoFornecedor DECIMAL(10,2),
    DataUltimoFornecimento DATETIME,
...
CREATE INDEX idx_cliente_produto ON ClienteProduto(ProdutoId);
CREATE INDEX idx_fornecedor_produto ON FornecedorProduto(ProdutoId);
...
INSERT INTO ClienteProduto (ClienteId, ProdutoId, DataUltimaCompra, QuantidadeComprada) VALUES
(1, 1, '2023-10-15 10:00:00', 1),
(1, 3, '2023-10-16 14:30:00', 2),
(2, 2, '2023-10-17 09:15:00', 3);

INSERT INTO FornecedorProduto (FornecedorId, ProdutoId, PrecoFornecedor, DataUltimoFornecimento) VALUES
(1, 1, 3000.00, '2023-10-01 08:00:00'),
(1, 3, 200.00, '2023-10-05 10:00:00'),
(2, 2, 100.00, '2023-10-10 09:00:00'),
(2, 1, 3200.00, '2023-09-28 11:00:00');
...
('SAIDA', 1, 1, NULL, 'Venda para cliente João Silva'),
('SAIDA', 2, 3, NULL, 'Venda para cliente João Silva'),
('SAIDA', 3, 2, NULL, 'Venda para cliente Maria Santos');
...
-- Todos os fornecedores de cada produto (relacionamento muitos-para-muitos)
SELECT p.Nome AS Produto, f.RazaoSocial AS Fornecedor, fp.PrecoFornecedor AS 'Preço Fornecedor'
FROM Produto p
JOIN FornecedorProduto fp ON p.Id = fp.ProdutoId
JOIN Fornecedor f ON fp.FornecedorId = f.Id;

-- Histórico de compras dos clientes
SELECT c.Nome AS Cliente, p.Nome AS Produto, cp.QuantidadeComprada, cp.DataUltimaCompra
FROM Cliente c
JOIN ClienteProduto cp ON c.Id = cp.ClienteId
JOIN Produto p ON cp.ProdutoId = p.Id;
...
SELECT 
    f.RazaoSocial AS Fornecedor,
    COUNT(DISTINCT fp.ProdutoId) AS 'Qtd Produtos Fornecidos',
    SUM(p.QuantidadeEstoque * fp.PrecoFornecedor) AS 'Valor Total em Estoque'
FROM Fornecedor f
LEFT JOIN FornecedorProduto fp ON f.Id = fp.FornecedorId
LEFT JOIN Produto p ON fp.ProdutoId = p.Id
GROUP BY f.Id;
